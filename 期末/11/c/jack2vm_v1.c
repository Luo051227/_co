...
// Note: SK_FIELD maps to "this" segment in VM
...
struct Parser {
    ...
    void (*compile_class_var_dec)(Parser*);
    void (*compile_subroutine)(Parser*);
    void (*compile_parameter_list)(Parser*);
    void (*compile_var_dec)(Parser*);
    void (*compile_statements)(Parser*);
    void (*compile_do)(Parser*);
    void (*compile_let)(Parser*);
    void (*compile_while)(Parser*);
    void (*compile_return)(Parser*);
    void (*compile_if)(Parser*);
    void (*compile_expression)(Parser*);
...
void lex_tokenize(Lex* lex, char* source);
...
void parser_compile_class(Parser* parser);
// ... 其他解析函式已在結構體中宣告
...
Token* lex_advance(Lex* lex) {
    ...
    return &lex->tokens[lex->token_count]; // Return EOF token
}
...
Symbol* symbol_table_lookup(SymbolTable* st, const char* name) {
    for (int i = 0; i < st->subroutine_symbol_count; i++) {
        if (strcmp(st->subroutine_symbols[i].name, name) == 0) {
            return &st->subroutine_symbols[i];
        }
    }
    for (int i = 0; i < st->class_symbol_count; i++) {
        if (strcmp(st->class_symbols[i].name, name) == 0) {
            return &st->class_symbols[i];
        }
    }
    return NULL;
}
...
//----------------------------------------------------------------------
void vm_writer_init(VMWriter* vm, const char* jack_filepath) {
    char vm_filepath[MAX_PATH_LENGTH];
    char out_dir[MAX_PATH_LENGTH];
...
void vm_writer_close(VMWriter* vm) {
    if (vm->outfile) fclose(vm->outfile);
}

void vm_writer_write_push(VMWriter* vm, const char* segment, int index) {
    fprintf(vm->outfile, "push %s %d\n", segment, index);
}

void vm_writer_write_pop(VMWriter* vm, const char* segment, int index) {
    fprintf(vm->outfile, "pop %s %d\n", segment, index);
}

void vm_writer_write_arithmetic(VMWriter* vm, const char* command) {
    fprintf(vm->outfile, "%s\n", command);
}

void vm_writer_write_label(VMWriter* vm, const char* label) {
    fprintf(vm->outfile, "label %s\n", label);
}

void vm_writer_write_goto(VMWriter* vm, const char* label) {
    fprintf(vm->outfile, "goto %s\n", label);
}

void vm_writer_write_if(VMWriter* vm, const char* label) {
    fprintf(vm->outfile, "if-goto %s\n", label);
}

void vm_writer_write_call(VMWriter* vm, const char* name, int n_args) {
    fprintf(vm->outfile, "call %s %d\n", name, n_args);
}

void vm_writer_write_function(VMWriter* vm, const char* name, int n_locals) {
    fprintf(vm->outfile, "function %s %d\n", name, n_locals);
}

void vm_writer_write_return(VMWriter* vm) {
    fprintf(vm->outfile, "return\n");
}
...
Token* parser_require_token(Parser* p, TokenType type, const char* value) {
    ...
    if (t->type != type) {
        char err_msg[256];
        snprintf(err_msg, sizeof(err_msg), "Expected token type %d, but got %d ('%s')", type, t->type, t->value);
        ERROR(err_msg);
    }
    if (value && strcmp(t->value, value) != 0) {
        char err_msg[256];
        snprintf(err_msg, sizeof(err_msg), "Expected '%s', but got '%s'", value, t->value);
        ERROR(err_msg);
    }
    return t;
}
...
int is_keyword(Parser* p, const char* kw) {
    return is_token(p, T_KEYWORD, kw);
}
int is_any_keyword(Parser* p, int count, ...) {
    va_list args;
    va_start(args, count);
    for (int i=0; i<count; ++i) {
        if(is_keyword(p, va_arg(args, const char*))) {
            va_end(args);
            return 1;
        }
    }
...
int is_sym(Parser* p, char c) {
    char s[2] = {c, '\0'};
    return is_token(p, T_SYM, s);
}
int is_any_sym(Parser* p, const char* syms) {
    Token* t = lex_peek(p->lex);
    if (t->type != T_SYM) return 0;
    return strchr(syms, t->value[0]) != NULL;
}

int is_type(Parser* p) {
    return is_token(p, T_ID, NULL) || is_any_keyword(p, 3, "int", "char", "boolean");
}
...
void compile_term(Parser* p) {
    // ##### FIX 1: Correctly pass p->lex to lex_peek #####
    Token* t = lex_peek(p->lex);
    ...
    if (t->type == T_NUM) {
        ...
        } else if (strcmp(t->value, "false") == 0 || strcmp(t->value, "null") == 0) {
            vm_writer_write_push(p->vm, "constant", 0);
        } else if (strcmp(t->value, "this") == 0) {
            vm_writer_write_push(p->vm, "pointer", 0);
        }
        lex_advance(p->lex);
    } else if (is_sym(p, '(')) {
        lex_advance(p->lex); // '('
        p->compile_expression(p);
        parser_require_token(p, T_SYM, ")");
    } else if (is_any_sym(p, "-~")) {
        char op = t->value[0];
        lex_advance(p->lex); // op
        p->compile_term(p);
        if (op == '-') vm_writer_write_arithmetic(p->vm, "neg");
        else vm_writer_write_arithmetic(p->vm, "not");
    } else if (t->type == T_ID) {
        char name[MAX_TOKEN_LENGTH];
        strncpy(name, t->value, sizeof(name) - 1);
        name[sizeof(name) - 1] = '\0';
        lex_advance(p->lex); // identifier

        if (is_sym(p, '[')) { // Array access
        ...
        } else if (is_sym(p, '(') || is_sym(p, '.')) { // Subroutine call
            char func_name[MAX_TOKEN_LENGTH * 2 + 2];
            int n_args = 0;
            if (is_sym(p, '.')) {
                lex_advance(p->lex); // '.'
                Token* subroutine_name = parser_require_token(p, T_ID, NULL);
                Symbol* s = symbol_table_lookup(p->symbols, name);
                if (s) { // method call on an object
                    vm_push_variable(p, name);
                    snprintf(func_name, sizeof(func_name), "%s.%s", s->type, subroutine_name->value);
                    n_args = 1;
                } else { // static function call
                    snprintf(func_name, sizeof(func_name), "%s.%s", name, subroutine_name->value);
                }
            ...
            parser_require_token(p, T_SYM, "(");
            n_args += p->compile_expression_list(p);
            parser_require_token(p, T_SYM, ")");
...
void compile_do(Parser* p) {
    ...
    p->compile_term(p); // This handles the subroutine call logic
...
void compile_let(Parser* p) {
    ...
    if (is_sym(p, '[')) {
        ...
        p->compile_expression(p);
        parser_require_token(p, T_SYM, "]");
        vm_writer_write_arithmetic(p->vm, "add"); // base + index
    }
    ...
    if (is_array) {
        vm_writer_write_pop(p->vm, "temp", TEMP_ARRAY); // pop value
        vm_writer_write_pop(p->vm, "pointer", 1);     // pop base+index to THAT
        vm_writer_write_push(p->vm, "temp", TEMP_ARRAY); // push value back
        vm_writer_write_pop(p->vm, "that", 0);         // pop to *(base+index)
    } else {
...
void compile_if(Parser* p) {
    ...
    if (has_else) {
        vm_writer_write_goto(p->vm, label_end);
    }
...
void compile_statements(Parser* p) {
    while (1) {
        // ##### FIX 2: Correctly pass p->lex to lex_peek #####
        Token* t = lex_peek(p->lex);
        if (t->type != T_KEYWORD) break;
        
        if (strcmp(t->value, "let") == 0) p->compile_let(p);
        else if (strcmp(t->value, "if") == 0) p->compile_if(p);
        else if (strcmp(t->value, "while") == 0) p->compile_while(p);
        else if (strcmp(t->value, "do") == 0) p->compile_do(p);
        else if (strcmp(t->value, "return") == 0) p->compile_return(p);
        else break; // Not a statement keyword
    }
}

void compile_var_dec(Parser* p) {
    lex_advance(p->lex); // 'var'
    Token* type = lex_advance(p->lex); // type
    do {
        Token* name = parser_require_token(p, T_ID, NULL);
        symbol_table_define(p->symbols, name->value, type->value, SK_VAR);
        if (is_sym(p, ',')) lex_advance(p->lex); else break;
    } while (1);
    parser_require_token(p, T_SYM, ";");
}
...
void compile_subroutine(Parser* p) {
    ...
    lex_advance(p->lex); // void or type
    Token* name = parser_require_token(p, T_ID, NULL);
...
void parser_compile_class(Parser* p) {
...
void parser_init(Parser* parser, const char* filepath) {
    ...
    parser->compile_class = parser_compile_class;
    parser->compile_class_var_dec = compile_class_var_dec;
    parser->compile_subroutine = compile_subroutine;
    parser->compile_parameter_list = compile_parameter_list;
    parser->compile_var_dec = compile_var_dec;
    parser->compile_statements = compile_statements;
    parser->compile_do = compile_do;
    parser->compile_let = compile_let;
    parser->compile_while = compile_while;
    parser->compile_return = compile_return;
    parser->compile_if = compile_if;
    parser->compile_expression = compile_expression;
    parser->compile_term = compile_term;
    parser->compile_expression_list = compile_expression_list;
    
    parser->compile_class(parser);
}
